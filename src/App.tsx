...
export default function QueryForm() {
  ...
  const [previousConversations, setPreviousConversations] = useLocalStorage<Message[]>("previousConversations", []);
  ...
  const handleQuerySubmit = async () => {
    ...
    try {
      ...
      setPreviousConversations((prev) => [...prev, ...[newMessage, responseMessage]]);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col items-center w-full p-10">
        ...
        <div className="space-y-2">
          {messages.map((message, index) => (
            <div key={index} className={`flex ${message.type === "query" ? "justify-end" : "justify-start"}`}>
              <Card className={`p-4 max-w-[75%] rounded-lg ${message.type === "query" ? "bg-blue-500 text-white" : "bg-gray-200"}`}>
      ...
      <div className="w-full max-w-4xl flex gap-4 justify-around items-center fixed bottom-20 bg-white z-10">
        <Input
          value={query}
        ...
        />
        <Button onClick={handleQuerySubmit} disabled={isLoading} className="w-auto">
          Submit
        </Button>
      </div>
